...
static int CompareInt(const void *elemA, const void *elemB)
{ 
  return (*(int *)elemA - *(int *)elemB); 
}

static void PrintInt(void *elem, void *fp)
{
  printf("%d\n", *(int *) elem);
}

static void PrintCstr(void *elem, void *fp)
{
  printf("%s\n", *(char **) elem);
}
...
int main(int ignored, char **alsoIgnored) 
{
  ...
  //MemoryTest();
  vector stvec;
  VectorNew(&stvec, sizeof(struct frequency), NULL, 0);
  struct frequency a;
  a.ch = 'a';
  a.occurrences = 1;
  VectorAppend(&stvec, &a);
  vector * vecPt = &stvec;
  printf("%d\n", VectorLength(vecPt));	
  ...
  /*
  vector intVector;
  VectorNew(&intVector, sizeof(int), NULL, 0);
  int val;
  for(val = 0; val < 10; val++) {
    VectorAppend(&intVector, &val);
  }

  VectorMap(&intVector, PrintInt, NULL);
  
  VectorSort(&intVector, CompareInt);
  int find = 15;
  const void * key = &find;
  int index = VectorSearch(&intVector, key, CompareInt, 0, false); 
  
  printf("%d\n", index);
  int n = VectorLength(&intVector);
  for(int i =0; i < n; i++) {
    printf("%d\n", *(int *)VectorNth(&intVector, i));
    }
  ...
  */ 
  /*
  const char *friends[] = {"Al", "Bob", "Carl", "Mike", "Kevin", "Brad", "Jim", "Frank", "Julie", "Jerry"};
  vector cstr;
  VectorNew(&cstr, sizeof(char *), FreeString, 0);
  int i;
  for(i = 0; i < 10; i++) {
    char *copy = strdup(friends[i]);
    VectorAppend(&cstr, &copy);
  }
  const char *repl = "John";
  char *cprepl = strdup(repl);
  //VectorInsert(&cstr, &cprepl, 1);
  //VectorInsert(&cstr, &cprepl, 1);
  //VectorDelete(&cstr, 6);
  //printf("%d\n", VectorLength(&cstr));
  
  //printing
  VectorMap(&cstr, PrintCstr, NULL);
  VectorDispose(&cstr);
  */
  return 0;
}
