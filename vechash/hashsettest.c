...
static void PrintFreq(void * elem, void * aux)
{
...
{/*
  ...
  HashSetEnter(&counts, &a);
  HashSetEnter(&counts, &b);
  HashSetEnter(&counts, &a);
   HashSetEnter(&counts, &a);
 HashSetEnter(&counts, &a);
  HashSetEnter(&counts, &c);
  HashSetMap(&counts, PrintFreq, NULL);
  struct frequency * freqPt = HashSetLookup(&counts, &e);
  if(freqPt != NULL) 
     printf("%c -- %d\n", freqPt->ch, freqPt->occurrences);
  printf("%d\n", HashSetCount(&counts));
  */
